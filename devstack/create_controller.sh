...
EXTERNAL_BRIDGE_MAC="f8:34:41:37:f1:e2"
INTERNAL_BRIDGE="internal"
INTERNAL_BRIDGE_MAC="f8:34:41:37:f1:e3"
...
write_files:
  - content: |
        ...
        DEBIAN_FRONTEND=noninteractive sudo apt-get install -qqy git || sudo yum install -qy git net-tools ntpdate vim nc
        sudo chown stack:stack /home/stack
        cd /home/stack
        git clone https://git.openstack.org/openstack-dev/devstack
        cd devstack
        echo '[[local|localrc]]' > local.conf
        echo ADMIN_PASSWORD=diego >> local.conf
        echo DATABASE_PASSWORD=grassato >> local.conf
        echo RABBIT_PASSWORD=diego >> local.conf
        echo SERVICE_PASSWORD=diego >> local.conf
        ./stack.sh
